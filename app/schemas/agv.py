...
class AGVCreate(BaseModel):
    ...
    name: str = Field(..., description="AGV 名稱/編號，如 AGV01")
    model: str = Field(..., description="AGV 型號：K400, Cargo, Loader, Unloader")
    description: Optional[str] = Field("N/A", description="AGV 描述，預設為 N/A")
    enable: int = Field(1, description="啟用狀態：1=啟用, 0=停用")
    parameter: Optional[Dict[str, Any]] = Field(None, description="AGV 參數設定（JSON 格式）")
